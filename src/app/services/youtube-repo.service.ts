...
import { Observable, Subject, catchError, map, of, switchMap, take, withLatestFrom } from 'rxjs';
...
export class YoutubeRepoService {
  ...
  getUserData(force = false): Observable<[User[], boolean, boolean, string]> {
    return this.store.select('userReducer').pipe(
      ...
      map(([_, userReducerState]) => [this.unNormalize(userReducerState.entities), userReducerState.loading, userReducerState.error, userReducerState.errorMsg])
    );
  }
  ...
  addUser(id:number, name:string, email:string):Observable<[User[], boolean, boolean, string]> {
   ...
   unNormalize(entities: { [id: string]: any }) {
    if (!entities) {
        return [];
    } else {
